...
public class Post implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
        ...
        SubjectEntity subjectOfSect1 = SubjectEntity.builder().subjectCode("CC05").subjectDescription("Com 5").section(sect1).status("Open").dateCreated(LocalDateTime.now()).build();
        SubjectEntity subjectOfSect2 = SubjectEntity.builder().subjectCode("PF1").subjectDescription("Event Driven programming architetura tanggggggggg").section(sect1).status("Closed").dateCreated(LocalDateTime.now()).build();
        
        SubjectEntity subjectOfSect3 = SubjectEntity.builder().subjectCode("CCsdf05").subjectDescription("ss 5").section(sect7).status("Closed").dateCreated(LocalDateTime.now()).build();
        SubjectEntity subjectOfSect4 = SubjectEntity.builder().subjectCode("PFsdf1").subjectDescription("ss Drivenz").section(sect3).status("Open").dateCreated(LocalDateTime.now()).build();
        SubjectEntity subjectOfSect5 = SubjectEntity.builder().subjectCode("CsdC05").subjectDescription("Cs 5").section(sect5).status("Open").dateCreated(LocalDateTime.now()).build();
